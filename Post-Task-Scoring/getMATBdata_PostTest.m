function [rate_data, sysmon_data, track_data, comm_data, resman_data, matb_data] = getMATBdata_PostTest(date_time, subject_folder)

%% RATE - get rate times
rate_file = strcat('RATE_',date_time,'.txt'); % change to "WRS_" for MATB-II Update
fid = fopen(fullfile(subject_folder,rate_file));
...
fid = fopen(fullfile(subject_folder,resman_file));
...
fid = fopen(fullfile(subject_folder,comm_file));
...
fid = fopen(fullfile(subject_folder,sysm_file));
...
fid = fopen(fullfile(subject_folder,trck_file));
...
matb_file = strcat('MATB_',date_time,'.txt');
fid = fopen(fullfile(subject_folder,matb_file));
