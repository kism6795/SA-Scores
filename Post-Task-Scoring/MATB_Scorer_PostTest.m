function matb_score = MATB_Scorer_PostTest(trial_num, subject_folder, date_time)
...
%{ 
    ...
    data has been moved to the D drive).
%}
...
load(fullfile(subject_folder,'events.mat'));
events = events2; clear events2

%% Import MATB Data
[rate_data, sysmon_data, track_data, comm_data, resman_data, matb_data] = getMATBdata_PostTest(date_time, subject_folder);
...
current_time = (rate_data.times(trial_num,1)*60 ...
                +rate_data.times(trial_num,2));
...
matb_score = scoreTrial(rate_data, sysmon_data, track_data, comm_data, ...
                        resman_data, events, flow_rates, last_datum, ...
                        current_datum);
...
if ~isempty(track_data)
    ...
    penalty_total = penalty_total + penalty_pertrack*sum(abs(temp_data(~isnan(temp_data))));
    clear temp_data;
end
%% scoreCOMM
...
if ~isempty(resman_data)
    ...
    % determine start & current time
    temp_time = resman_data.times(current_datum(4),:);
    current_time = floor(temp_time(1)*60+temp_time(2));
    temp_time = resman_data.times(last_datum(4),:);
    last_current_time = floor(temp_time(1)*60+temp_time(2));

    % set initial flow rates for tanks A & B
    pumps_on = pump_status(1,:) == 1;
    flow_A = zeros(1,current_time);
    flow_B = zeros(1,current_time);
    flow_A(1) = -800;
    flow_B(1) = -800;
    
    for j = 2:current_time  % calculates fuel levels for all trial time up until now
        ...
        if count <= length(resman_data.times) && j>=(resman_data.times(count,1)*60+resman_data.times(count,2))  
            ...
            % calculate the fuel levels based on flow rates when no other info available
            fuel_levels(j,:) = fuel_levels(j-1,:) + [flow_A(j), flow_B(j)].*1/60;
        end
    end
    
    % cuts down to just the last trial f fuel levles
